...
import cn.edu.shu.ankai.sqlite.CollectColumns;
import cn.edu.shu.ankai.sqlite.DBManager;
import cn.edu.shu.ankai.sqlite.ErrorColumns;
import cn.edu.shu.ankai.sqlite.ExamErrorColumns;
import cn.edu.shu.ankai.sqlite.ExamResultColumns;
...
public class ResultActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		add = (TextView) findViewById(R.id.add_test);
	...
	private class QueryTask extends AsyncTask<Void, Void, ArrayList<CauseInfo>> {
		...
		protected void onPostExecute(ArrayList<CauseInfo> list) {
			...
			for (int i = 0; i < list.size(); i++) {
			 ...
			 useTime = "" + TimeUtils.secToTime(60 * 10 - time);
			 hisResult = "" + intDadui * 5 + "分";
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == COMPLETED) {
				if (json.equals("true")){
					Toast.makeText(ResultActivity.this,"成绩上传成功",Toast.LENGTH_LONG).show();
				}
				else{
					Toast.makeText(ResultActivity.this,json,Toast.LENGTH_LONG).show();
	...
	public class JsonDownloadThread extends Thread {
		public void run() {
			...
			InputStream inStream = this.getClass().getResourceAsStream("/assets/add_test.xml");
			try {
				json = getMobileAddress(inStream, key, exam_name, curTime+"",useTime,hisResult,"0");
			Log.e("步骤",json);
	...
	private String getMobileAddress(InputStream inStream, String key, String user,String testTime,String useTime,String grade,String time) throws Exception {
		String soap = readSoapFile(inStream, key, user, testTime,useTime,grade,time);
		...
		if (conn.getResponseCode() == 200) {
			...
			return JsonCommon.parseResponseXML(Stream, "AddAppTestGradeInfoResult");
		}
		Log.e("步骤1", conn.getResponseCode() + "");
		return conn.getResponseCode() + "";
	}

	private String readSoapFile(InputStream inStream, String key, String user,String testTime,String useTime,String grade,String time) throws Exception {
